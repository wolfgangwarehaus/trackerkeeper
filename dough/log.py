...
Level comes from the ``DOUGH_LOG`` env var (``debug`` / ``info`` / ``warning``,
default ``info``) — nothing is persisted in Settings; a support request is
"relaunch with DOUGH_LOG=debug", not a toggle hunt. ``run_app`` calls
...
def _level_from_env() -> int:
    """Root level from ``DOUGH_LOG`` (debug/info/warning/error), default INFO."""
    name = (os.environ.get("DOUGH_LOG") or "info").strip().upper()
...
def log_file_path() -> Path | None:
    ...
    (``dough.diagnostics`` tails this for the support report.)"""
    return _file_path
