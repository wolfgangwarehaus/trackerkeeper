...
``dough.__version__`` — and the user hasn't dismissed that exact version — it
...
stamp (the ``DOUGH_CHANNEL`` env var, default ``"source"`` → checks ON, the safe
direction — a store package sets ``DOUGH_CHANNEL=msix``/``aur``/… to disable).
...
from dough.platform_compat import is_macos_sandboxed, is_msix_packaged
...
def get_channel() -> str:
    ...
    to ``DOUGH_CHANNEL`` (default ``"source"``). Never raises."""
    try:
        if is_msix_packaged():
            return "msix"
        ...
        pass
    return (os.environ.get("DOUGH_CHANNEL") or "").strip().lower() or "source"
