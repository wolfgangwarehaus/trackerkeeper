...
See dough/theme.body_color_for() and docs/research/portable_blur.md.
...
Wayfire, SwayFX), the user can target dough in their own window rules:
our Wayland app_id is the stable string "dough" (set via
setDesktopFileName).
"""
...
if IS_LINUX:
    from dough.blur import _kwin as _backend
elif IS_WINDOWS:  # pragma: no cover - exercised on Windows
    from dough.blur import _dwm as _backend
elif IS_MACOS:  # pragma: no cover - exercised on macOS
    from dough.blur import _macos as _backend
else:  # pragma: no cover
    from dough.blur import _unsupported as _backend
