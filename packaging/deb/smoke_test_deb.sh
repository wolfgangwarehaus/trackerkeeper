...
# by `dough bake` — edit the template, never this file.
...
test -x /usr/bin/dough && test -x "$(readlink -f /usr/bin/dough)"
echo "OK: /usr/bin/dough resolves to the bundle"

# Metadata validators (catch build_deb.sh staging drift). appstream is advisory.
desktop-file-validate /usr/share/applications/io.github.wolfgangwarehaus.dough.desktop
echo "OK: .desktop validates"
appstreamcli validate --no-net /usr/share/metainfo/io.github.wolfgangwarehaus.dough.metainfo.xml || true
...
set +e
QT_DEBUG_PLUGINS=1 QT_QPA_PLATFORM=xcb xvfb-run -a timeout 15 /usr/bin/dough >/tmp/boot-xcb.log 2>&1
rc=$?
set -e
if [ "$rc" != 124 ] && [ "$rc" != 0 ]; then
