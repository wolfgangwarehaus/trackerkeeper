"""AppWindow — trackerkeeper's frosted, frameless, cross-platform main-window base.
...
from trackerkeeper.design_tokens import RADIUS_WINDOW
...
class AppWindow(QMainWindow):
    ...
    def __init__(self, *, title: str = "trackerkeeper"):
        super().__init__()
        ...
        # trackerkeeper/blur/_faux_frost.py.
        from trackerkeeper.blur._faux_frost import FauxFrost
        ...
        # (full-size content view, see trackerkeeper/macos_window.py) so the top bar
        # clears the floating traffic lights while the frosted backdrop shows
        # through behind them.
        if IS_MACOS:
            from trackerkeeper.macos_window import TITLEBAR_INSET
    ...
    def _make_top_bar(self, title: str):
        ...
        change). Default: the generic :class:`~trackerkeeper.top_bar.TopBar`."""
    ...
    def _faux_frost_active(self) -> bool:
        ...
        opaque override (``TRACKERKEEPER_OPAQUE``, main-window-only). Replaces the flat
        near-opaque panel with the frosted material."""
        from trackerkeeper import blur, ui_helpers
    ...
    def _apply_win_native_frame(self):
        if not self._win_frameless:
            return
        try:
            from trackerkeeper import win_frameless
    ...
    def _first_blur_pass(self):
        ...
        self._refresh_body_color()
        # TRACKERKEEPER_BLUR_DIAG=1 dumps the full surface truth once the window is
        ...
        if os.environ.get("TRACKERKEEPER_BLUR_DIAG") == "1":
            try:
        ...
        if IS_MACOS:
            from trackerkeeper.blur import _macos
