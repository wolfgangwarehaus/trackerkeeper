"""trackerkeeper's build-time metadata — the one source the oven renders every manifest from.

Build-time counterpart to :mod:`trackerkeeper.identity` (runtime). ``identity`` owns the
LIVE app identity (org / app / display_name) and the reverse-DNS id projections
the RUNNING QApplication needs; this module reads the ``[tool.trackerkeeper.metadata]``
sidecar in ``pyproject.toml`` — the descriptive / store fields a future
``trackerkeeper bake`` stamps into the ``.desktop`` entry, the AppStream metainfo, deb
...
class MetadataError(RuntimeError):
    """The ``[tool.trackerkeeper.metadata]`` sidecar is missing or malformed."""
...
def _find_pyproject() -> Path:
    ...
    for parent in Path(__file__).resolve().parents:
        ...
        if candidate.is_file():
            return candidate
    raise MetadataError(
        "pyproject.toml not found above trackerkeeper/metadata.py — the baking-phase "
...
@lru_cache(maxsize=1)
def load() -> dict:
    """The raw ``[tool.trackerkeeper.metadata]`` table, with the resolved package
    ...
    try:
        meta = dict(data["tool"]["trackerkeeper"]["metadata"])
    except KeyError as exc:
        raise MetadataError(f"[tool.trackerkeeper.metadata] missing from {pyproject}") from exc
    ...
    # the Beat-2 `trackerkeeper bake` renderer — until then metadata_version is a forward
    ...
    if "metadata_version" not in meta:
        raise MetadataError(f"[tool.trackerkeeper.metadata] has no metadata_version in {pyproject}")
    for key in ("org_slug", "app_slug", "github_owner", "repo_name"):
        if key not in meta:
            raise MetadataError(f"[tool.trackerkeeper.metadata] missing required key {key!r} in {pyproject}")
    ...
    # the slug is taken on PyPI (trackerkeeper publishes as `trackerkeeper-base`). Folded in from
