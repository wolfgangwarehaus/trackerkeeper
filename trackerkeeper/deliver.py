"""``trackerkeeper deliver`` — the Delivery-phase walkthroughs (docs/DELIVERY.md).
...
performed.
    ...
    python -m trackerkeeper.deliver              # the board: every channel, its state
    python -m trackerkeeper.deliver pypi         # walk ONE channel's next steps
    python -m trackerkeeper.deliver --release    # the update lap (the Improvements loop)
...
class Ctx:
    ...
    # the bare name is squatted (trackerkeeper publishes as trackerkeeper-base; the import and
    ...
    def pypi_state(self) -> str | None:
        ...
        reads as LIVE (found live: PyPI 'trackerkeeper' is a 2015-era third-party
        package)."""
