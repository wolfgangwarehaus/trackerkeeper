...
trackerkeeper keeps a tiny, generic settings surface: theme, accent, font scale,
...
``trackerkeeper.identity`` (or via ``trackerkeeper.configure(...)``); this reads from there.
"""
...
class Settings:
    ...
    def follow_system_accent(self) -> bool:
        ...
        # read at launch + watched live by trackerkeeper.system_accent. Off by default.
    ...
    def auto_hide_scrollbars(self) -> bool:
        ...
        SETTING is trackerkeeper-base; an app wires install_autofade_scrollbars on its own
        scroll areas when this is on (trackerkeeper can't know which scroll areas an app has)."""
    ...
    def language(self) -> str:
        ...
        # locale). Read by trackerkeeper.i18n at boot; restart-applied.
    ...
    def check_for_updates(self) -> bool:
        # Daily GitHub latest-release check (trackerkeeper.updates) — the chip in the
