...
*value* still lives on its original module (``trackerkeeper.ui_helpers``,
``trackerkeeper.design_tokens``, ``trackerkeeper.icons``); this module owns the
...
TOKENS: dict[str, ColorToken] = {
    ...
    "ACCENT": ColorToken(
        ...
        module="trackerkeeper.ui_helpers",
    ),
    ...
    "DANGER": ColorToken(
        ...
        module="trackerkeeper.design_tokens",
    ),
...
def _cascade_accent_family(accent_value: Any, *, persist: bool) -> None:
    ...
    if not (isinstance(accent_value, str) and accent_value.startswith("#")):
        return
    try:
        from trackerkeeper.theme import (
            _BORDER_ALPHAS,
            _border_accent_for,
            _darken,
            get_active_theme,
...
def import_palette(palette: dict) -> int:
    ...
    # instead of ~15 (one per apply_override). A whole palette swap then costs a
...
def _suspend_emit():
    ...
    whole palette costs a SINGLE app-wide re-stamp instead of ~15 (one per
