...
# TRACKERKEEPER_WIN_GLASS_ALPHA, then bake the value you like.
...
def _win_glass_alpha() -> int:
    ...
    ``TRACKERKEEPER_WIN_GLASS_ALPHA=40`` (lighter) … ``=160`` (heavier)."""
    try:
        v = int(os.environ.get("TRACKERKEEPER_WIN_GLASS_ALPHA", str(_WIN_BODY_DEFAULT_ALPHA)))
    except ValueError:
...
# was tuned by eye against KDE Plasma's blur. Tune live with TRACKERKEEPER_MAC_GLASS_ALPHA.
...
def _mac_glass_alpha() -> int:
    ...
    ``TRACKERKEEPER_MAC_GLASS_ALPHA=90`` (lighter) … toward the theme base (heavier)."""
    try:
        v = int(os.environ.get("TRACKERKEEPER_MAC_GLASS_ALPHA", str(_MAC_BODY_DEFAULT_ALPHA)))
    except ValueError:
...
class Theme:
    ...
    # separates Frosted from Solid. Applied via trackerkeeper/blur/; a silent
    ...
    # body_color_for() and trackerkeeper/blur/status().
...
def body_color_for(theme: "Theme", status, surface: str = "main") -> tuple:
    ...
    ``status`` (a ``trackerkeeper.blur.BlurStatus``).
    ...
    if not theme.blur or theme.fallback_body_alpha is None:
        return base
    # Import here so theme.py stays importable before trackerkeeper.blur is ready
