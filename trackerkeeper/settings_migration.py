...
/ theme-axes one-shots that healed real installs); trackerkeeper ships the generic
...
The add-a-migration recipe (a fork's, or trackerkeeper's own):
...
def _open_qsettings() -> QSettings:
    ...
    pair ``trackerkeeper.settings.Settings`` opens. The (format, scope, org, app) ctor
