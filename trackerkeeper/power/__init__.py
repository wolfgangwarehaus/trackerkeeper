...
and :meth:`SleepInhibitor.release` when it's done. trackerkeeper owns the cross-platform
...
Backend (selected once per process, mirroring ``trackerkeeper.autostart`` /
``trackerkeeper.notifications``):
...
def _select_backend() -> ModuleType:
    ...
    from trackerkeeper.platform_compat import IS_LINUX, IS_WINDOWS

    if IS_WINDOWS:
        from trackerkeeper.power import _windows as backend
    elif IS_LINUX:
        from trackerkeeper.power import _linux as backend
    else:
        from trackerkeeper.power import _unsupported as backend
...
class SleepInhibitor(QObject):
    ...
    double as Qt signal slots), trackerkeeper provides the cross-platform HOW. Idempotent:
