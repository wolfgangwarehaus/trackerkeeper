...
Installs trackerkeeper's bundled scripted effect into the user's KWin effects
...
running app's identity (``trackerkeeper.identity.app()``) as it copies the effect into
place, so a fork matches *its own* windows with no source edit and no `trackerkeeper
...
# Bundled template source: trackerkeeper/drag_repaint/effect/dragrepaint/. The dir is
