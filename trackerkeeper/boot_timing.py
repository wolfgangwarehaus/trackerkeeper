"""Boot-phase wall-clock instrumentation, opt-in via ``TRACKERKEEPER_BOOT_TIMING=1``.
...
Usage on any install:
    TRACKERKEEPER_BOOT_TIMING=1 trackerkeeper            (fish/bash)
    $env:TRACKERKEEPER_BOOT_TIMING="1"; trackerkeeper    (PowerShell)
"""
...
logger = logging.getLogger("trackerkeeper.boot")

_ENABLED = os.environ.get("TRACKERKEEPER_BOOT_TIMING") == "1"
_T0 = time.perf_counter()
_LAST = _T0


def mark(label: str) -> None:
    """Record a boot milestone. No-op unless TRACKERKEEPER_BOOT_TIMING=1."""
    global _LAST
    if not _ENABLED:
        return
