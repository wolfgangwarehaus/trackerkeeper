...
``dev/update_translations.sh`` extracts them into ``trackerkeeper/i18n/*.ts``
...
Two translators install per non-English language:
  ...
  * The app catalog (``trackerkeeper_<lang>.qm``) shipped as package data in
    ``trackerkeeper/i18n/``. The literal ``trackerkeeper`` in the catalog name is deliberate:
    ``trackerkeeper new``'s whole-word identity replace renames it with the package,
...
# Keep in sync with trackerkeeper/i18n/trackerkeeper_<code>.ts — the Settings dropdown builds
# from this list. trackerkeeper ships none; a fork adds a row per catalog, e.g.:
#     ("es", "Spanish", "Español"),
...
def install(app) -> str:
    ...
    try:
        ...
        qm = res.files("trackerkeeper.i18n").joinpath(f"trackerkeeper_{lang}.qm")
