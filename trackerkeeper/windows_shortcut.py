...
pip's entry-point launcher (``trackerkeeper.exe``) is a distlib stub whose
...
- Render ``%LOCALAPPDATA%/trackerkeeper/trackerkeeper.ico`` from the
...
- Write a per-user Start Menu shortcut (no elevation needed):
  ``%APPDATA%/Microsoft/Windows/Start Menu/Programs/trackerkeeper.lnk``
...
couple of ``Path.exists()`` calls. ``TRACKERKEEPER_NO_START_MENU_SHORTCUT=1`` opts
out (and a source-checkout ``python -m trackerkeeper`` run has no launcher
...
def _render_icon(path: Path) -> bool:
    ...
    try:
        ...
        from trackerkeeper.ui_helpers import make_app_icon
...
_STAMP_SENTINEL = "TRACKERKEEPER_STAMP_OK"
...
def _write_shortcut(lnk: Path, exe: Path, ico: Path) -> bool:
    ...
    try:
        ...
        if r.returncode != 0 or _STAMP_SENTINEL not in out:
            # INFO (not debug) so the `python -m trackerkeeper` console run
...
def sync() -> None:
    ...
    if not IS_WINDOWS or os.environ.get("TRACKERKEEPER_NO_START_MENU_SHORTCUT"):
        return
    exe = _launcher_exe()
    if exe is None:
        return
