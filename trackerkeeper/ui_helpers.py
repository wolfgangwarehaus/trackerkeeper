...
from trackerkeeper.design_tokens import rad
from trackerkeeper.icon_button import IconButton
from trackerkeeper.platform_compat import IS_MACOS

# ── Theme ────────────────────────────────────────────────────────────────────
# Palette + body fills come from the active Theme (trackerkeeper/theme.py).
# Constants are re-exported so existing `from trackerkeeper.ui_helpers import
...
from trackerkeeper.theme import get_active_theme, ink_alpha  # noqa: F401  (re-exported)
...
# (trackerkeeper/theme.py). The constants below are flat re-exports of those
...
def body_color_tuple(surface: str = "main") -> tuple:
    ...
    *verified* behind the window — glass (~67%) when it is, a near-opaque
    ...
    main / mini / dialog. Does NOT apply the main window's TRACKERKEEPER_OPAQUE override
    (that's main-window-only; the caller handles it). Never raises."""
    from trackerkeeper import blur
    from trackerkeeper.theme import body_color_for, get_active_theme
...
def frosted_fallback_active() -> bool:
    ...
    paint the faux-frost backdrop (``trackerkeeper/blur/_faux_frost.py``). Shared by the
    main window and mini player so they fall back identically. (The main window
    additionally suppresses it under the TRACKERKEEPER_OPAQUE override.)"""
    from trackerkeeper import blur
...
def apply_font_settings_live() -> None:
    ...
    from trackerkeeper import design_tokens as _dt
...
def _build_global_style() -> str:
    ...
    from trackerkeeper.design_tokens import RADIUS_LG, RADIUS_MD, RADIUS_SM, rad
    ...
    # NB hover tooltips are drawn by our custom popup (trackerkeeper/custom_tooltip),
...
QToolTip {{
    /* Hover tooltips are drawn by trackerkeeper/custom_tooltip (a top-level
...
def refresh_theme() -> str:
    ...
    ``from trackerkeeper.ui_helpers import ACCENT`` keeps a STALE reference,
    ...
    try:
        from trackerkeeper import color_tokens as _ct
...
# a surface might have imported `from trackerkeeper.ui_helpers import …` and
...
def _propagate_theme_constants() -> None:
    """Rebind the theme-token constants in every ``trackerkeeper.*`` module
    ...
    ``from trackerkeeper.ui_helpers import TEXT`` and bake it into a
    ...
    for mod_name, mod in list(sys.modules.items()):
        if mod is None or mod is src or not mod_name.startswith("trackerkeeper."):
            continue
...
def _propagate_font_constants() -> None:
    ...
    ``trackerkeeper.*`` module that imported them by value — the font-scale analog
    ...
    for mod_name, mod in list(sys.modules.items()):
        if mod is None or mod is _dt or not mod_name.startswith("trackerkeeper."):
            continue
        for name, value in values.items():
            if hasattr(mod, name):
                setattr(mod, name, value)
...
def apply_app_palette() -> None:
    ...
    text, which is invisible on a light trackerkeeper theme (the dark
    ...
    custom popup (trackerkeeper/custom_tooltip), which carries its own colour.
...
def _load_app_icon_svg_bytes() -> bytes:
    ...
    try:
        ...
        res = _ir.files("trackerkeeper.assets").joinpath("trackerkeeper.svg")
        if not res.is_file():
            return b""
...
def _draw_placeholder_icon(size: int) -> QPixmap:
    ...
    try:
        from trackerkeeper.theme import get_active_theme as _gt
...
def make_app_icon(size: int = 64) -> QPixmap:
    """trackerkeeper logo, rasterized from the bundled
    ``trackerkeeper/assets/trackerkeeper.svg`` at the requested pixel size.
...
class EmptyState(QWidget):
    ...
    def _apply_styling(self) -> None:
        ...
        from trackerkeeper.design_tokens import TYPE_BODY, TYPE_CAPTION, type_qss
...
class CircleSwatch(QPushButton):
    ...
    trackerkeeper and butterPDF). This paints with QPainter instead: the fill is one
...
def frost_scroll_surface(area) -> None:
    ...
    trackerkeeper hallmark. Generic over any ``QAbstractScrollArea`` (QScrollArea, QListView,
    a Qt PDF view, …)."""
    area.setStyleSheet(
