"""``trackerkeeper new`` — scaffold a fresh OWNED app from this trackerkeeper checkout.

``trackerkeeper-new <slug> --display "My App" --org <owner> [--summary "…"]`` turns THIS
clone of trackerkeeper into an owned ``<slug>`` app: it renames the package, rewrites every
trackerkeeper identity reference, mints a fresh installer id, strips trackerkeeper's own dev
...
trackerkeeper*: the one command from a trackerkeeper checkout to a real app repo.

Identity is centralized (the seam + the ``[tool.trackerkeeper.metadata]`` sidecar), so the
...
def _replace_in_tree(root: Path, pairs: list[tuple[str, str]]) -> None:
    ...
    pair's UPPER_SNAKE env-var prefix (``TRACKERKEEPER_*`` → ``BUTTERPDF_*``) — ``_``
    ...
    # …and the lowercase_snake prefix (``trackerkeeper_<code>.qm`` i18n catalog
...
def _identity_org_repairs(
    ...
    case: trackerkeeper's own org and owner coincide ("wolfgangwarehaus"), so the
...
def _scaffold(root: Path, slug: str, display: str, new_org: str, new_owner: str, summary: str | None) -> None:
    ...
    # about trackerkeeper-the-base survives the whole-word trackerkeeper→slug rewrite.
    ...
    #    (io.github.…trackerkeeper.desktop), which the content-replace can't touch, so the
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="trackerkeeper-new",
        description="Scaffold a fresh OWNED app from this trackerkeeper checkout (destructive, in-place).",
    )
    ...
    if not (root / sidecar["app_slug"]).is_dir():
        parser.error(f"no package dir {sidecar['app_slug']}/ at {root} — run inside a trackerkeeper checkout")
