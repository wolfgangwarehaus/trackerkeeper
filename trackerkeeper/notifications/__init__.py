"""Cross-platform desktop notifications. Public API is platform-agnostic;
the actual implementation lives in a per-OS backend module.

Public API:
    is_supported() -> bool   # backend can actually display notifications
    notify(title, body="", icon=None, app_name=None, tag=None) -> None
        # app_name=None resolves to trackerkeeper.identity.app() (the live app slug)

Linux: shells out to `notify-send` (libnotify), which routes through the
freedesktop.org `org.freedesktop.Notifications` D-Bus service. Picked up
by KDE Plasma, GNOME Shell, dunst, mako, swaync, etc.

Windows: Action-Center toasts via the `windows_toasts` package (WinRT
`ToastNotificationManager`), bound to trackerkeeper's stamped AppUserModelID.

macOS: real Notification Center banners via `UNUserNotificationCenter`
inside the signed `.app`, falling back to an `osascript` banner for
from-source runs — see `_macos`.

If a platform backend can't be imported (package/runtime missing) the
selector degrades to the unsupported backend, whose `notify()` is a
silent no-op and `is_supported()` returns False so call sites can gate
UX cleanly.

Failures from the active backend (D-Bus down, no notification daemon
running, notify-send missing mid-process) never raise to the caller —
notifications are best-effort by design.
"""

from __future__ import annotations

from types import ModuleType

_backend: ModuleType | None = None


def _select_backend() -> ModuleType:
    """Resolve the backend module once per process and memoize. Mirrors
    the dispatch shape used by `trackerkeeper.autostart`."""
    global _backend
    if _backend is not None:
        return _backend

    from trackerkeeper.platform_compat import IS_LINUX, IS_MACOS, IS_WINDOWS

    if IS_LINUX:
        from trackerkeeper.notifications import _linux as backend
    elif IS_WINDOWS:
        try:
            from trackerkeeper.notifications import _windows as backend
        except Exception:
            from trackerkeeper.notifications import _unsupported as backend
    elif IS_MACOS:
        try:
            from trackerkeeper.notifications import _macos as backend
        except Exception:
            from trackerkeeper.notifications import _unsupported as backend
    else:
        from trackerkeeper.notifications import _unsupported as backend

    _backend = backend
    return _backend


def is_supported() -> bool:
    return _select_backend().is_supported()


def notify(
    title: str,
    body: str = "",
    icon: str | None = None,
    app_name: str | None = None,
    tag: str | None = None,
) -> None:
    """Show a desktop notification. Silent no-op on unsupported
    platforms; never raises. ``app_name`` defaults to the live app identity
    (``trackerkeeper.identity.app()``) — pass it only to override. ``tag`` groups
    successive notifications so a new one *replaces* the prior in place
    rather than stacking."""
    try:
        _select_backend().notify(title, body, icon, app_name, tag)
    except Exception:
        pass
