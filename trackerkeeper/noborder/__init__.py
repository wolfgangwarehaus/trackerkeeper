"""KWin ``noborder`` window rule for trackerkeeper's borderless main window.

On KDE Wayland trackerkeeper's main window is deliberately **server-side-decorated**
...
The rule is matched on the app's **wmclass** (``trackerkeeper.identity.app()``, the
...
exact title; trackerkeeper matches wmclass so the title stays free.)
...
feature — are intentionally not ported; trackerkeeper has no generic use for one yet.
...
if is_kde_wayland():
    from trackerkeeper.noborder import _kwin as _backend
else:
    from trackerkeeper.noborder import _unsupported as _backend
