...
OFF by default. Only stood up when ``TRACKERKEEPER_TEST_BRIDGE=1`` is set at launch
(wired in ``trackerkeeper.app.run_app``; the fork transform renames the env prefix).
...
TRACKERKEEPER_TEST_BRIDGE=1. It must never be enabled in a shipped/packaged build.
"""
