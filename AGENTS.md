...
write invalid TOML), and the open window live-reloads each change:
    ...
    trackerkeeper-breadboard list [phase]                     # ids + state — read first
    trackerkeeper-breadboard add <phase> "text" [--priority now|next|later] [--note "…"]
    trackerkeeper-breadboard check <id> [--off] [--note "…"]  # done / reopen (stamps you)
    trackerkeeper-breadboard note <id> "text"
    trackerkeeper-breadboard priority <id> now|next|later     # move a baking card
    trackerkeeper-breadboard rm <id>
    trackerkeeper-breadboard request --clear                  # after you fulfil agent_request
